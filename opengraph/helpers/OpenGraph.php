...
class OpenGraph implements \Iterator
{
  ...
  * 
  */
  static public function favicon($url) {
      ...
      if ( count($arr) == 0 )
      {
        $arr = $xml->xpath('//link[@rel="apple-touch-icon-precomposed"]');
      }

      if ( count($arr) == 0 )
      {
        $service = "https://www.google.com/s2/favicons?domain=".$url;
        return $service;
      }
   ...
   */
	static private function _parse($HTML) {
        ...
        $key = strtr(substr($tag->getAttribute('property'), 3), ':', '_'); // Naveed added this line.
      ...
      if ($tag->hasAttribute('name') && $tag->getAttribute('name') === 'logo') {
          var_dump("Found");
          die();
          $logo = $tag->getAttribute('content');
      }
